...
st.title("🧑‍💼 Real-time Employee Misconduct Detection")
...
with st.spinner("Loading models..."):
    # YOLO models
    yolo_model_path = r"C:\Users\Lenovo\Desktop\FINAL\yolov8n.pt"
    ...
    # Eye-state model
    eye_model_path = r"C:\Users\Lenovo\Desktop\FINAL\eye_state_model.h5"
...
def is_eyes_closed(face_crop):
    if face_crop.size == 0:
        return False
    h, w, _ = face_crop.shape
    eye_region = face_crop[int(0.2*h):int(0.5*h), int(0.15*w):int(0.85*w)]
    ...
    return pred < 0.5  # True if eyes closed
    ...
    try:
        ...
        for box in results.boxes:
            ...
            if label == "person":
                ...
                # Gender
                gender, _ = predict_gender(face_crop)

                # Sleeping
                sleeping = is_eyes_closed(face_crop)
                display_text = f"{gender}"
                if sleeping:
                    display_text += " | Sleeping"
                    color = MISCONDUCT_COLORS["Sleeping"]
                else:
                    color = (0, 255, 0)
            ...
            elif label in ["cell phone", "phone"]:
                phone_detected = True
